...
1. **iOS relay protocol implementation**: Same changes as the Android relay implementation above, adapted for Swift/CryptoKit and the iOS `ShareTransport` protocol.
2. **Android UX — group Distributed tab by `secretId`**: The Distributed tab currently shows one entry per share (one per recipient), so a 2-of-2 deposit to Bob and Carol produces two entries. A future improvement is to group shares by `secretId` and show a single row per logical secret, expanding to show per-holder status on tap.
3. **iOS biometric unlock**: The Android app gates `reconstruct()` behind `BiometricPrompt`. The iOS `ShareDetailView` currently reconstructs immediately; it should gate via `LAContext.evaluatePolicy(.deviceOwnerAuthenticationWithBiometrics)` from the `LocalAuthentication` framework.
4. **End-to-end testing**: Test Android ↔ iOS interop (Android deposits a share, iOS recipient approves retrieval, Android reconstructs) against a live `sbt run` backend. Blocked on item 1 above.
