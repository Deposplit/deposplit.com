...
3. **Defense in depth — recipient-side signature verification**: Currently, the deposplit.com relay is the sole enforcer of Ed25519 signature validity. As a defense-in-depth measure, recipients should independently verify the sender's Ed25519 signature on every incoming request (PickUp, Retrieve, Delete) before acting on it, rejecting anything that does not verify against the sender's public key stored in the local contact record. This makes signature verification a recipient guarantee rather than a relay trust assumption — consistent with the server-blindness philosophy — and is a prerequisite for BYOR (see below).
4. **BYOR — Bring Your Own Relay** (requires item 3): Privacy- and security-conscious users (or organizations) may prefer to relay shares through infrastructure they control rather than through deposplit.com. BYOR allows a relay backend to be configured per contact: Alice routes her shares to Bob via an Airtable table, while Carol uses the default deposplit.com relay. A third-party table carries the same columns as `share_requests`; a company can provision one table for all its employees. Because the relay is already a driven port (`ShareRelay`) in the hexagonal architecture, each backend is a separate adapter (deposplit.com REST API, Airtable API, Google Sheets API, or a self-hosted deposplit.com instance). The relay URL and any necessary credentials are exchanged out-of-band alongside the contact's public keys — encoded in the QR code or contact link. Security properties are preserved: the relay remains an opaque ciphertext store; recipient-side signature verification (item 3) catches spoofed rows that a passive-table relay cannot reject server-side; the consent model is enforced by the apps, not the table.
