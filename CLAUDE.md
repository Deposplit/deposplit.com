...
Each contact record stores: a stable local `contactId` (item 7 — survives key changes, anchors `HeldShare`/`ShareMetadata`), Ed25519 public key, X25519 public key, pseudonym, verification level, date verified, a per-key compromised/revoked history (item 10 — `revokedEdKeys`), the timestamp of the last key change (item 10 — `keyChangedAt`), and (BYOR) an optional per-contact `relayBaseUrl` override. All stored locally on the device. Item 8 added `ContactManagement.updateContact(contactId, newKeys?, newLevel?)`, a contact-update-in-place primitive used both for benign key rotation/recovery relinking and, on Android/iOS, a "Relink (Key Changed)" UI action.

Adding a contact is the natural moment to prompt for in-person QR verification.

### Contact Verification

> ⚠ **Item 10 extends this** with a per-key *compromised/revoked* flag and a `min(level, LOW)` downgrade applied to an auto-accepted rotation — implemented, see item 10 below.
...
5. **Freemium one-time unlock (optional, future)**: A single **one-time in-app purchase** ("Premium") that both removes the deposit cap and unlocks sender-side BYOR. Enforcement is **client-side only** (consistent with the server-blindness philosophy — the backend never learns payment status), and therefore honor-system by design (a patched/rogue client bypasses it) — keep the paywall light-touch, no heavy voucher/gifting infrastructure. The free/premium line:
   ...
   - **Gating sender-side BYOR removes the always-available local-relay-testing path** (Aug 2026 relay-default consistency cleanup: `RelayDefaults`/`RelaySettings` on both platforms assume the Settings screen's default-relay editor is unconditionally reachable for pointing a dev build at a local `sbt run` instance). Once this item ships, both `Android/CLAUDE.md` (next to `SKIP_BIOMETRIC`) and `iOS/CLAUDE.md` (next to the biometric bypass discussion) will need their own debug-only fake-Premium `PurchaseRepository`/`isPremium()` bypass to restore that, same shape as `SKIP_BIOMETRIC` — harmless given enforcement is already client-side/honor-system by design (see above).
...
8. ~~**Identity recovery — holder-driven metadata reconstitution (pure-social).**~~ — **done.** Resolves the "Identity Recovery" section's `k`-of-`n`-vs-single-approver TBD *and* the metadata half left open by item 7. Decided during the spec walk.
   ...
   **No migrations** — Deposplit is pre-launch; test relays and test devices will be reset to a clean slate.
9. ~~**Holder-key-change handling + share-redundancy monitoring.**~~ — **done.** Post-item-7 a holder's key change is no longer a "share lost" event — it splits in two, each needing a different mechanism.
   - ~~**Proactive rotation (holder keeps data):**~~ — **done, 2026-08-21.** Only Alice's *routing pointer* to the holder goes stale (the plaintext share is safe). The holder still has the old key, so their app pushes a **signed `rotate(K_old → K_new)`** to contacts via the relay; Alice auto-verifies against the trusted old key and updates the contact record **in place, preserving `contactId`**. Fully automatic — *on the receiving end*. The trigger — a user deliberately regenerating their own keypair while still holding the device and old keys (the opposite case from item 8's device-*loss* recovery) — was originally deferred as its own follow-up, since it needed a new `IdentityStore` capability and a Settings UI action that didn't exist yet; both are now built. `Identity` gained `generateNewKeyPair()` (pure key generation, not persisted) and `activateKeyPair(keyPair)` (persists via the existing, unchanged `IdentityStore.save`, preserving the pseudonym); `ShareManagement` gained `regenerateIdentity()`, which best-effort drains the inbox/distributed state under the *old* identity, generates new keys, pushes a signed rotation notice to every contact via the **unchanged** `pushRotation` (still signing as the old identity, proving continuity), then activates the new keys — order is load-bearing, since a mid-flight crash leaves the old identity live and a retry self-corrects. **Investigation finding surfaced while designing this:** the relay authorizes every `/share-requests` call by matching a row's embedded key against the *caller's currently-authenticating* key, so any relay row still addressed to/from the old key at the moment of rotation becomes permanently unreachable afterward (not deleted, just invisible) — mitigated with the best-effort drain plus a documented residual-risk warning in the confirmation UI, not a grace-period dual-key polling mechanism, matching this project's existing "relay is a best-effort mailbox" philosophy (item 12). Surfaced in the Settings screen as a plain-confirmation (not biometric-gated) "Regenerate My Identity" action. Shipped on Android, iOS, and (for consistency, not full parity — no Settings UI) phon; see `TODO.md` item 9 for full implementation notes.
...
10. ~~**Malicious key substitution + stolen-key revocation.**~~ — **done.** Closes the key-change thread. Decided during the spec walk.
    ...
    **Implemented** on Android `:hexagon`/`:app`, iOS `hexagon`/`Deposplit`, and (for cross-platform *consistency*, not full parity) `deposplit.com/hexagons/phon` — **100% client-local, no relay work at all**, since every piece (the revoked-key flag, the conflict record, the gating check, the retrieve-approval indicator) lives entirely in local state. The `min(level, LOW)` downgrade rule was already shipped as part of item 9's `processRotations()` on every platform; item 10 added the rest. `Contact` gained `revokedEdKeys` (a growing historical **set**, not a single boolean — so a later legitimate relink to a genuinely new key is never blocked by an old flag) and `keyChangedAt` (stamped by `updateContact` on any key change, surfaced as the retrieve-approval indicator). A new `KeyConflict` value object + `KeyConflictRepository` driven port hold the durable local record of a detected conflict — **captured before the corresponding relay notice is deleted**, so the alert never depends on relay retention (the relay may lose its state at any time). `processRotations()` now checks the incoming notice's `oldEd25519Key` against `contact.revokedEdKeys` *before* the existing downgrade/auto-accept branch: on a match it saves a `KeyConflict`, deletes the relay notice, and skips `updateContact` entirely (the contact record is left untouched — never auto-resolved); no match falls through to the unchanged item-9 auto-accept path. `ContactManagement.markKeyCompromised(contactId, edPublicKey?)` (idempotent, defaults to the contact's current key) and `ShareManagement.listKeyConflicts`/`dismissKeyConflict` are the new driving-port primitives.

    Android and iOS additionally gained full UI: a warning badge + "Mark Key Compromised" action + confirmation dialog on the Contacts screen; a "Key Conflicts" section on the Requests screen (Dismiss-only — deliberately no "Accept" button, since the only legitimate way forward is a fresh human-verified relink through the *existing* item-8 Relink flow, not a second, weaker acceptance pathway) above pending requests; and an orange "this requester's key changed N days ago" indicator on retrieval requests specifically (the attack signature is key change → quick retrieval, so the nudge is scoped to that transaction type only). `phon` picked up the data-model and gating logic for consistency but has no UI for any of it (no compromise-flag button, no conflict-list view, no key-changed indicator), matching every prior item's precedent for phon's minimal HTMX views.

    **No migrations** — Deposplit is pre-launch; test relays and test devices will be reset to a clean slate.

    **Work items:** tracked in `TODO.md` (item 10).
...
12. ~~**Polling, staleness & relay-TTL cadence — the custodial-heartbeat model.**~~ — **done.** Decided during the spec walk. Pins down how items 5 (polling) and 9 (redundancy monitoring) cash out into concrete cadence rules, and **flips item 9's health-check from a pull to a push.** No new relay behavior — the relay stays a blind mailbox; this is client cadence + one message-type reshape.
    ...
    **Implemented everywhere (2026-08-18): relay, Android, iOS, and (for consistency, not full parity) phon.** The relay gained a `custody_heartbeats` table (`id, holder_key, owner_key, secret_ids, opted_out, signature, created_at`, `UNIQUE (holder_key, owner_key)`) with its own `CustodyHeartbeats` port and `POST`/`GET /custody-heartbeats` endpoints — like `inventory`/`key_rotations`, no `state` column, but unlike either it is neither consumed-and-deleted nor self-approved: `pushHeartbeat` **upserts** (H2 lacks Postgres's `ON CONFLICT ... DO UPDATE`, so the Anorm adapter does a portable select-existing-id-then-insert-or-update instead), and `listHeartbeats` returns without deleting, since a heartbeat is a standing "last seen" status a holder re-emits, not a one-shot delivery. `PayloadCanonical.forHeartbeat(ownerKey, secretIds, optedOut)` sorts `secretIds` before joining so the signed bytes don't depend on list-construction order; the same construction covers the opt-out notice.

    Every client gained: a `CustodyHeartbeatTuning` object pinning the three UI-tuning constants (`emissionInterval = 3 days`, `lossThreshold = 3×`, `staleWarningThreshold = 2×`); `Contact.heartbeatOptedOutAt`/`lastHeartbeatSentAt`/`heartbeatEmissionOptedOut`; `ShareMetadata.lastConfirmedAt`; a `RetainedDepositBlob` value object + `RetainedDepositRepository` driven port implementing the deposit-retention rule (`deposit()` now retains each holder's encrypted blob until first-observed pickup confirmation, then discards it — safe because each blob is encrypted to the *holder's* X25519 key, so the sender can't decrypt it herself); and `ShareManagement.setHeartbeatEmissionOptedOut`. `ShareService` gained `emitHeartbeats()` (holder side, called from `syncInbox()` — coalesces one signed heartbeat or opt-out notice per distinct sender once that sender's emission interval has elapsed; `lastHeartbeatSentAt` only advances on a *successful* push, so a transient relay outage retries on the very next poll) and `processHeartbeats()` (owner side, called from `syncDistributed()` — auto-verifies each holder's latest heartbeat against the contact's trusted key, captures/clears `heartbeatOptedOutAt` durably and locally the instant it's observed since the relay may lose its state at any time, and stamps `lastConfirmedAt` on any matching `ShareMetadata`, discarding the retained blob if still pending). `syncDistributed()`'s deposit-approval loop now gates the freshness stamp on whether the retained blob is still pending, not on the row's `Approved` state alone — so a long-dead holder's unchanging Approved row is never re-stamped fresh on a later poll; a retrieval-approval poll was added purely for its freshness side effect.

    Android and iOS additionally gained: a `FreshnessBucket` (`Confirmed`/`Unmonitored`/`SilentOverdue`) computed on each `HolderStatus`, feeding a freshness-gated `n_live` on `SecretGroup.health`; stale/unmonitored/silent-overdue labels on the Distributed tab; and a per-contact "Pause/Resume Heartbeats" action on the Contacts screen (a badge when opted out). phon picked up the full data model and `ShareService` gating logic — including deposit retention and freshness stamping — for cross-platform *consistency*, not full parity: no heartbeat toggle button or freshness badge, since its minimal HTMX views have no plumbing for either yet (same precedent as items 9–11).
...
14. ~~**Crypto agility — self-describing keys, bundled cipher suites, and per-message transport tagging.**~~ — **done.** Decided during a second spec walk (Aug 2026), prompted by the relay's `PublicKey`/`X25519Key` opaque types hard-coding 32-byte Ed25519/X25519 assumptions directly into their names and length validation, and `PayloadCanonical`'s signed byte constructions carrying no algorithm/version tag at all. The goal is not a pluggable-cipher marketplace — no evidence of demand for per-user algorithm choice — but making a **future fleet-wide algorithm swap** (the realistic driver: Ed25519/X25519 are not post-quantum-safe, and a long-lived-secrets app is exactly the shape of system exposed to "harvest now, decrypt later") additive rather than a breaking wire-format migration. Pre-launch is the moment to bake this in cheaply — retrofitting it after real contacts have pinned real untagged keys would mean every key in the wild is "legacy untagged format," the exact flag-day mess agility exists to avoid.
    ...
    **Bundled `cipherSuite`, not two independent per-key tags.** `Contact` gains one `cipherSuite` field — a wire-tagged value, string-style like `ShareTransactionType.wireValue` rather than a raw enum ordinal, since ordinals aren't safe across three independently hand-ported enums — covering the *matched pair* of signing algorithm + agreement algorithm currently in force for that contact, e.g. `"ed25519+x25519-v1"` as the sole value that exists today. Rejected: two independent algorithm tags, one per key. Both of a device's keypairs are already generated together at first launch and rotated together (item 9); decoupling them would only invite invalid sign-algorithm/agreement-algorithm combinations to reject, for no expressed use case.

    **`key_rotations` schema follows the rename and gains `new_cipher_suite`.** `old_ed25519_key`/`new_ed25519_key`/`new_x25519_key` → `old_verify_key`/`new_verify_key`/`new_enc_key`; a new `new_cipher_suite` column carries the rotated identity's algorithm pairing. No `old_cipher_suite` column: the recipient already has it pinned on the existing `Contact` record being rotated away from — the same "the old value is already known context, only the new value is genuinely new information" principle used for the transport tag below. `PayloadCanonical.forRotation` appends `newCipherSuite` to its signed byte sequence (append-only, per item 8's precedent for keeping the cross-platform fixed-seed vector test stable everywhere else) — so a forged cipher-suite claim in a rotation notice is caught exactly like a forged key. Item 10's `min(level, LOW)` downgrade rule extends to trigger on a cipher-suite-only change too, with no key-value change required: an algorithm change is still "continuity of key control, not a personhood assurance," the same reasoning item 10 already applies to a plain key rotation.

    **Ciphertext gets its own, lighter-weight, per-message tag — deliberately not folded into `cipherSuite`.** The transport construction (X25519 agreement → HKDF-SHA-256 → ChaCha20-Poly1305) has three algorithm slots, but only the KDF+AEAD pair needs an in-band tag. The agreement algorithm is already unambiguous by the time encryption starts: the sender can't even perform key agreement without first knowing the recipient's `cipherSuite` (agreement is only defined between matching algorithms), and the recipient already knows their own current keypair's algorithm — it's theirs. Only "which KDF/AEAD did the sender's software happen to apply" is genuinely new information originating solely on the sender's side, so only that needs to travel with the message. Folding the agreement algorithm into the ciphertext tag too would be pure redundancy (it must always equal what `Contact.cipherSuite` already says, or something's already wrong) with no benefit — one authoritative place per fact.

    Because item 7 already re-derives each deposit/retrieval leg fresh (never pinned to a past key), the transport tag needs **no persistent state and no rotation/trust mechanism at all** — a device just always encrypts with its current preferred `TransportSuite`, and a decrypting device dispatches on the tag it reads. This is a much cheaper case than the identity side: the wire format becomes self-describing — `suiteTag || nonce || ciphertext+tag` in place of today's `nonce || ciphertext+tag` — and because the whole `ciphertext` blob is already fully included in `PayloadCanonical.forOpen`'s and `forRespond`'s signed bytes, the tag rides inside the signature **for free**: no new column on `share_requests`, no new field appended to `PayloadCanonical`. The relay never decrypts, so this change is entirely invisible to it — contrast with the identity-signing side below. A device that reads a tag it doesn't recognize must fail with a clear, typed, user-visible error ("this share used an encryption scheme this app version doesn't support"), never a silent misparse. Cost: one extra tag byte per ciphertext, forever, in exchange for the agility.

    **Relay involvement is asymmetric.** The relay actively verifies `senderSignature`/`recipientSignature` (item 3) and the `key_rotations`/`custody_heartbeats` signatures server-side — it is *not* blind to the signing algorithm — so `PublicKey.verify()` in `hexagons/relay` needs to become algorithm-dispatching too (a trivial single-branch dispatch today, an explicit extension point for whenever a second signing algorithm exists). `X25519Key` and the transport suite, by contrast, stay entirely opaque to the relay, exactly as ciphertext already is — no relay change follows from either.

    **Variable-length keys.** `PublicKey`/`X25519Key`'s hard-coded `KeyLength = 32` check has to go — a future signing algorithm (e.g. a PQ scheme) or KEM will not share Ed25519/X25519's key/signature size. Cheap now, before any real key is pinned to the fixed length; worth a follow-up sanity check on QR-code data density (item 6's in-person "Very High" verification channel) once a concrete larger algorithm is on the table, not a blocker today.

    **QR/link contact-exchange payload gains `cipherSuite`.** The out-of-band key-exchange payload (currently `v:2`, per item 4's `relay` field addition) needs a `cipherSuite` field alongside the renamed key fields — asserted by the presenting device at first-contact time, the same TOFU trust model the raw key bytes already carry (no rotation-signature needed here, this is the initial pin, not a change). The version bump (e.g. to `v:3`) and whether the JSON field names themselves get renamed alongside `ed`/`x` are implementation calls, not spec ones.
